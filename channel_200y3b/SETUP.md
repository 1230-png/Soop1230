...
**채우기 전에 `scripts/affiliate.py`를 읽으세요.** 대가성 고지는 선택이 아니고,
이 값이 채워지는 순간 사람 확인 없이 발행됩니다. 예전 코드는 이렇게 붙였습니다:

```python
full_description = f"{description}\n\n📚 추천 상품: {COUPANG_LINK}"
```

고지가 한 글자도 없습니다. 지금은 `affiliate.with_disclosure()`를 거치게 되어
있어 고지가 **맨 앞에** 붙고, 설명란에 손으로 적어 넣은 제휴 링크는
`affiliate.assert_disclosed()`가 잡아 업로드를 멈춥니다. 고지를 맨 앞에 두는
이유는 유튜브가 설명란을 몇 줄 뒤부터 접기 때문입니다 — 접힌 자리에 있는
고지는 표시하지 않은 것과 같습니다.

문구가 **적법한지는 코드가 보증하지 않습니다.** 공정거래위원회 「추천·보증 등에
관한 표시·광고 심사지침」과 쿠팡 파트너스 약관이 요구하는 표현은 바뀔 수 있으니
게시 전에 partners.coupang.com에서 직접 확인하세요. 코드가 보장하는 것은
"고지 없이 나가지는 않는다" 하나입니다.

참고로 이 저장소에 있던 금융 채널(머니로직)에는 제휴 링크를 넣지 않았습니다.
그쪽 면책 문구가 "추천하거나 권유하지 않습니다"라고 약속하고 있어서 수수료를
받는 링크와 정면으로 어긋났기 때문입니다. 채널을 접으면서 그 코드도 함께
지웠지만, 판단 기준은 남겨 둡니다 — **채널이 무엇을 약속했는지가 제휴 링크를
넣어도 되는지를 가른다.** 이 채널(@200-y3b)은 영어 표현을 가르칠 뿐 중립성을
약속하지 않으므로, 고지만 제대로 붙으면 문제가 없습니다.
...
## 문장집 (구독자 수와 무관한 수익 경로)

`scripts/phrasebook.py`가 표현 뱅크로 인쇄용 문장집을 만듭니다. 파트너
프로그램은 구독자와 시청 시간을 채워야 열리지만 이것은 **임계값이 없습니다.**
내용도 이미 있어서 새로 만들 것이 없습니다.

```bash
cd scripts
python3 phrasebook.py --exclude-used --out 문장집.html   # 영상으로 나간 것 빼고
python3 phrasebook.py --topic "공항·비행" --limit 50      # 주제별 소책자
```

브라우저에서 인쇄(Ctrl+P) → "PDF로 저장" 하면 배포용 파일이 됩니다. PDF
라이브러리를 붙이지 않은 이유는 한글 폰트를 또 한 벌 관리하게 되기 때문입니다
(`CLAUDE.md`가 matplotlib을 막은 것과 같은 이유). 브라우저 인쇄는 시스템에
깔린 폰트를 그대로 씁니다.

**무엇을 담을지는 장사의 문제입니다.** 영상으로 이미 공짜로 푼 것을 그대로
묶으면 살 이유가 없어서, 고르는 축만 만들어 뒀습니다. `--exclude-used`는
`used_log.csv`에 남은 발행 기록을 빼고, `--topic`은 주제별로 자릅니다.
가격·판매처는 정해진 것이 없습니다.
