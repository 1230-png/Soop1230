"""만들어 둔 한 편을 유튜브에 올린다.
    ...
    python3 channel_cs/upload.py --dir channel_cs/build/2026-09-21-E01

`channel_jp/upload.py` 에서 갈라져 나왔고, 그쪽이 지키는 네 가지를 그대로
지킨다. 전부 앞 채널들에서 한 번씩 데인 자리다.

1. **대체 자격 증명이 없다.** `longform/upload.py:40` 은 `Y3B_*` 가 없으면
   공용 `YT_*` 로 넘어간다. 여기는 `CS_*` 뿐이고, 없으면 멈춘다. 남의
   자격 증명으로 올라가느니 멈추는 편이 낫다.
2. **채널 ID 를 코드에 박지 않는다.** `CS_CHANNEL_ID` 가 없으면 자격 증명이
...
def youtube_client(env):
    ...
    try:
        ...
        # 무엇을 해야 하는지가 거기 없다.
        raise SystemExit(
            f"{PREFIX}_* 자격 증명(「다시 배우는 CS」)으로 인증하지 못했다: "
            f"{error}\n"
...
PUBLISHED_FIELDS = ("published_at", "episode", "season", "video_id",
...
def published_row(meta: dict, video_id: str, privacy: str, now: str) -> dict:
    ...
    return {
        ...
        "scene_count": str(meta.get("scene_count", "")),
        "privacy": privacy,
...
def video_body(meta: dict, privacy: str = "") -> dict:
    ...
    언어를 `ko` 로 적는다. 화면의 코드는 영어지만 말과 설명이 한국어이므로,
    한국어 시청자에게 추천되어야 한다.
    """
    return {
        "snippet": {
