...
1. **대체 자격 증명이 없다.** 저쪽은 `Y3B_*` 가 없으면 공용 `YT_*` 로
   넘어간다. 여기는 `JPN_*` 뿐이고, 없으면 멈춘다. 남의 자격 증명으로
   올라가느니 멈추는 편이 낫다(CLAUDE.md).
2. **채널 ID 를 코드에 박지 않는다.** 채널을 아직 안 만들었다. `JPN_CHANNEL_ID`
   가 없으면 자격 증명이 실제로 가진 채널 ID 를 **찍어 주고 멈춘다** — 그
   값을 그대로 시크릿에 넣으면 된다. 짐작해서 올리지 않는다.
    ...
    **대체 경로를 두지 않는다.** 공용 `YT_*` 로 넘어가게 해 두면 이 채널의
    ...
    if missing:
        raise SystemExit(
            ...
            "이 채널 전용 값이다. 공용 YT_* 나 머니로직의 MV_* 로 돌아가지 "
            "않는다 — 남의 채널로 올라가느니 멈춘다.")
    return values
