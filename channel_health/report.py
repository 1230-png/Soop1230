...
# 잡았더니 영상이 7편뿐이던 채널에서는 영영 켜지지 않았다(최근 5편을 빼면
# 2편만 남는다). 그 채널은 접었지만 기준을 되돌리지 않는다 — 새 채널은
# 언제나 그 상태에서 시작하고, 그때가 바로 이 검사가 필요한 시기다.
# 사람에게 "되짚어 보라"고 말하는 문장이지 무언가를 자동으로 바꾸는 신호가
# 아니라서, 늦게 켜지는 쪽보다 가끔 헛짚는 쪽을 택했다.
SLUMP_RATIO = 0.5
BASELINE_MIN = 2
