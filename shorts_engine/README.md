...
> **지금 돌고 있는 것은 이쪽이 아니다.** 매일 발행은 `channel_food/`가 하고,
> 피드백 루프는 거기서 CSV로 돌아간다. 하루 1편에는 그쪽이 맞다.
>
> 이 디렉토리는 **규모가 커질 때를 위한 판**이다. Ollama가 대본을 무한히 만들어
> 큐에 쌓고 여러 워커가 동시에 꺼내 가는 구조가 필요해지면 여기로 옮긴다.
> 그 전까지는 참고 구현으로 남겨 둔다 (테스트는 계속 통과한다).
>
> 컷 계산은 `channel_food/scripts/timeline.py` 한 곳에만 있고 여기서는 재노출만 한다.
> 두 파이프라인이 다른 컷을 만드는 일을 막기 위해서다.
