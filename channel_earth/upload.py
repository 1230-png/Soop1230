"""만들어 둔 한 편을 유튜브에 올린다.
    ...
    python3 channel_earth/upload.py --dir channel_earth/build/2026-09-21-day-short

`channel_jp` 계보를 그대로 잇는다. 대체 자격 증명 없음, 채널 ID 확인,
올리기 직전 재검사, 중복 업로드 차단.

여기에는 하나가 더 있다. **가짜 데이터로 만든 빌드를 막는다.** 개발용
픽스처가 실제 지진 정보인 척 올라가는 것은 다른 실수와 무게가 다르다.
막는 곳은 build.verify_publishable 한 곳이고, 여기서 그것을 다시 부른다.
"""
...
# `RUSH` 는 @Rush22 에서 온 글자지만 **그 채널을 접은 것이 아니다.** 방향만
# 바꾼 것이라 같은 유튜브 채널이 지금 「지구의 오늘」이다. 채널 이름을 바꿔도
# **채널 ID 는 그대로**이므로 이 자격 증명이 그대로 맞는다.
#
# 그러면 왜 `EARTH_*` 로 바꿔 달지 않았나 — **GitHub 시크릿은 값을 다시 읽을
# 수 없다.** 이 저장소에 `RUSH_*` 가 이미 있으면 그대로 쓰는 쪽이 사람 손을
# 하나도 안 쓴다. channel_jp 가 `MV_*` 를 남겨 둔 것과 같은 판단이다.
# 여기를 읽고 "접은 채널 것이 잘못 남았나" 싶거든, 아니다 — 이 채널 것이 맞다.
#
# 값이 낡았을 가능성은 코드가 막는다. 아래 youtube_client 가 인증한 뒤 **자격
# 증명이 실제로 가진 채널 ID 를 확인**하고 RUSH_CHANNEL_ID 와 다르면 멈추므로,
# 낡은 시크릿은 엉뚱한 업로드가 아니라 또렷한 오류가 된다.
#
# 채널마다 제 구글 클라우드 프로젝트를 쓴다 — 유튜브 일일 할당량이 채널이
# 아니라 **프로젝트 단위**라, 돌려쓰면 서로의 몫을 깎는다.
PREFIX = "RUSH"
...
def youtube_client(env):
    ...
    try:
        ...
        # 무엇을 해야 하는지가 거기 없다.
        raise SystemExit(
            f"{PREFIX}_* 자격 증명(「지구의 오늘」)으로 인증하지 못했다: "
            f"{error}\n"
...
PUBLISHED_FIELDS = ("published_at", "episode", "window", "video_id",
                    "duration_seconds", "quake_count", "privacy", "title")
